...
11. Create cost item. For substrates IN the 4-item interior catalog above, read unitCost/unitPrice from payload.catalog[substrate][coats] × tier multiplier (same as Stage C). For substrates NOT in the interior catalog (stucco, fascia, soffit, eaves, trim, railings, metal, cabinets, etc.), call search_catalog ONCE with the substrate word ALONE — a single word like "stucco", "fascia", "soffit", "eaves", "railing", "trim", "cabinet". NEVER prepend "exterior" or "interior", and NEVER search by location words like "body" or "walls" — catalog item names are substrate + condition + coats only; location lives in code_name and won't match. Then narrow the returned matches with the coats filter ("1-Coat" / "2-Coats" / "Prime + Paint : 2-Coats") and the condition, and pick the best match from those results to get organizationCostItemId, costCodeId, unitId, unitCost, and unitPrice. Multiply unitCost AND unitPrice by the tier multiplier. Do NOT re-search for "body", "walls", "exterior", or any other location term — one substrate-word search plus the coats filter is the answer. ONLY if that single search returns an empty result set may you ask the user for organizationCostItemId + costCodeId + unitCost + unitPrice. Never leave unitCost/unitPrice null.
